...
# 转换函数
def h5_to_pb(h5_model,out_prefix="output_",input_prefix="input_"):
    out_nodes = {}
    input_nodes = {}
    ...
    # }
    convert(sess,input_nodes,out_nodes)
...
def convert(sess,inputs,output):
    # 保存转换好的模型目录
    saveModDir = "models"
    mk_dir(saveModDir)
    # 每次转换都生成一个版本目录
...
def main():
    ...
    # log_path = "models/mask_rcnn_coco_0078.h5"
    log_path = "models/mask_rcnn_coco_0078.h5"

    # 加载网络模型
    mask_model = MaskRCNN(train_flag=False)
    # 加载权重模型
    mask_model.load_weights(log_path, by_name=True)

    h5_to_pb(mask_model.keras_model)
